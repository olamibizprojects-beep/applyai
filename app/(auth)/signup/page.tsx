...
export default function SignupPage() {
  ...
  const benefits = ['3 free cover letters per month', 'No credit card required', 'Cancel anytime']
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#0F172A]">Create your free account</h1>
          <p className="text-[#64748B] text-sm mt-1">Start landing more interviews today</p>

          <div className="flex flex-wrap justify-center gap-3 mt-4">
            {benefits.map((b) => (
              <span key={b} className="inline-flex items-center gap-1.5 text-xs text-[#10B981] font-medium">
                <Check size={12} />
                {b}
              </span>
            ))}
        ...
        <div className="bg-white rounded-2xl border border-[#E2E8F0] p-8 shadow-sm">
          {emailSent ? (
            <div className="text-center py-4">
              ...
              <h2 className="font-semibold text-[#0F172A] mb-2">Almost there!</h2>
              <p className="text-[#64748B] text-sm">We sent a sign-in link to <strong>{email}</strong>. Click it to activate your free account.</p>
            </div>
          ) : (
            <>
              <Button
              ...
              <p className="text-xs text-center text-[#94A3B8] mt-4">
                ...
                <Link href="/terms" className="underline hover:text-[#64748B]">Terms</Link>
                {' '}and{' '}
        ...
        <p className="text-center text-sm text-[#64748B] mt-6">
          ...
          <Link href="/login" className="text-[#3B82F6] font-medium hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  )
